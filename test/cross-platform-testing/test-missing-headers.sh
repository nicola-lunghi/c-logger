...
# Test 1c: Test with completely missing time.h header (HAVE_TIME_H=0)
echo "1c. Testing with HAVE_TIME_H=0 (should use our fallback time functions)..."
mkdir -p build-no-time-h || exit 1
cd build-no-time-h || exit 1
rm -rf CMakeCache.txt CMakeFiles/ cmake_install.cmake Makefile
cmake ../../.. -DCMAKE_C_FLAGS="-DHAVE_TIME_H=0 -DHAVE_SYS_TIME_H=0 -DHAVE_GETTIMEOFDAY=0 -DHAVE_LOCALTIME_R=0" -DBUILD_TESTS=ON -DWARNINGS_AS_ERRORS=ON
echo "Build output (should work with time.h fallback in logger_platform.c):"
make 2>&1 | head -20
cd .. || exit 1
echo
...
# Test 6: Functional test - verify timestamp format works
echo "6. Functional test - verifying timestamp format..."
mkdir -p build-functional-test || exit 1
cd build-functional-test || exit 1
rm -rf CMakeCache.txt CMakeFiles/ cmake_install.cmake Makefile
cmake ../../.. -DBUILD_EXAMPLES=ON -DWARNINGS_AS_ERRORS=ON
make 2>&1 | head -20
echo "Running console example to verify timestamp functionality:"
./example/logger_console_example 2>&1 | head -5
echo "Running file example to verify timestamp functionality:"
./example/logger_file_example
if [ -f "log.txt" ]; then
    echo "Log file contents (first few lines):"
    head -3 log.txt
    rm -f log.txt
else
    echo "Warning: log.txt not found!"
fi
cd .. || exit 1
echo
...
echo "1c. HAVE_TIME_H=0 with fallback (tests our new time.h detection)"
...
echo "6. Functional test - verifies timestamp format works correctly"
